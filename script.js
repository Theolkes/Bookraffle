...
<head>
  ...
  <style>
    ...
    .ação, .action { background: #f97316; }
    .drama { background: #eab308; }
    .ficção, .fiction { background: #22c55e; }
    .mistério { background: #0ea5e9; }
    .crítica { background: #d97706; }
    .política { background: #6b7280; }
    .histórico { background: #e17055; }
...
Dom Casmurro"></textarea><br />
  ...
  <div id="desc">Descrição...</div>
  <button id="copyBtn" onclick="copiarDescricao()" style="display:none;">📋 Copiar Descrição</button>

  <div id="assinatura">Feito por Miguel Ferreira Corrêa</div>
  ...
  <script>
    ...
    function limpaTag(tag) {
      return tag.trim().toLowerCase().replace(/[^\wáàâãéèêíïóôõöúçñ]/g, '');
    }
    ...
    async function sortearLivro() {
      ...
      // Animação sorteando por 3s, mudando livro a cada 100ms
      ...
      try {
        ...
        let descOriginal = livro.description || "Sem descrição disponível.";
